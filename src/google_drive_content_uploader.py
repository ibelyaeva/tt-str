...
def main(content_dir, parent_id, meta):
    logger = create_logger()
    logger.info('Starting Content Uploader for content folder: %s ; Parent Id: %s Only Metatada: %s ', str(content_dir), str(parent_id), str(meta))
    metadata_folder = init_metadata(content_dir, logger)
    client = gcl.GoogleClient(logger, metadata_folder)
    
    try:
        folder_id = client.create_folder_rec(content_dir, parent_id)
        if meta == 1:
            content = client.generate_folder_content(folder_id)
            generate_metadata(content, client, folder_id, logger)
...
if __name__ == "__main__":
    ...
    main(content_dir, parent_id, meta) 
